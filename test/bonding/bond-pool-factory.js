...
describe("BondPoolFactory contract", function () {
    ...
    describe("updateParams", function () {
        ...
        it("updateParams with wrong discount", async function () {
            await expect(bondPoolFactory.updateParams(200000, 10001, 200000)).to.be.revertedWith("BondPoolFactory.updateParams: DISCOUNT_OVER_100%");
        });

        it("updateParams with wrong vestingTerm", async function () {
            await expect(bondPoolFactory.updateParams(200000, 20, 129500)).to.be.revertedWith("BondPoolFactory.updateParams: MUST_BE_LONGER_THAN_36_HOURS");
        });
